...
IEC_PRA_TXD      = %00000100   ; PRA bit 2 — User-port TXD output. Kept high (released) in our masks.
IEC_PRA_ATN_OUT  = %00001000   ; PRA bit 3 — IEC ATN (output via inverter). 0=assert ATN, 1=release.
IEC_PRA_CLK_OUT  = %00010000   ; PRA bit 4 — IEC CLOCK (output via inverter). 0=clock low (assert), 1=release.
IEC_PRA_DATA_OUT = %00100000   ; PRA bit 5 — IEC DATA  (output via inverter). 0=data low (assert), 1=release.
IEC_PRA_CLK_IN   = %01000000   ; PRA bit 6 — IEC CLOCK input (read only). Polled via BIT during sync.
...
IEC_MASK_ATN_RELEASE  = IEC_PRA_ATN_OUT | IEC_PRA_TXD       ; PRA bits set high for ATN & TXD.
															; NOTE: With active-low bus this *releases* ATN at the connector.
															; Name reflects PRA bit state, not bus polarity — verify intent.

IEC_MASK_PRESET_BEFORE_SYNC = IEC_PRA_DATA_OUT | IEC_PRA_TXD  	; Preset before sync: DATA and TXD high (released). Does not touch ATN.
																; (“Ready To Send” staging mask used by iec_send_cmd.)

; Receive phases (two samples per call): odd = CLOCK high, even = CLOCK low
IEC_RECV_ODD  		= IEC_PRA_CLK_OUT | IEC_PRA_ATN_OUT | IEC_PRA_TXD  	; Drive PRA: clock high, ATN/TXD high (released) while sampling bit #1.
IEC_RECV_EVEN 		= IEC_PRA_ATN_OUT | IEC_PRA_TXD                  	; Drive PRA: clock low (by omitting IEC_PRA_CLK_OUT), ATN/TXD high for bit #2.

; Transmit phases (two emitted bits per call), select DATA based on C after ROL iec_shiftreg
IEC_SEND_ODD_D1   	= IEC_PRA_DATA_OUT | IEC_PRA_CLK_OUT | IEC_PRA_ATN_OUT | IEC_PRA_TXD  	; ODD phase: clock high, DATA=1 (released), ATN/TXD high.
IEC_SEND_ODD_D0 	= IEC_PRA_CLK_OUT  | IEC_PRA_ATN_OUT | IEC_PRA_TXD                  		; ODD phase: clock high, DATA=0 (assert) via cleared DATA bit.
IEC_SEND_EVEN_D1  	= IEC_PRA_DATA_OUT | IEC_PRA_ATN_OUT | IEC_PRA_TXD                 		; EVEN phase: clock low,  DATA=1 (released), ATN/TXD high.
IEC_SEND_EVEN_D0	= IEC_PRA_ATN_OUT  | IEC_PRA_TXD                                 		; EVEN phase: clock low,  DATA=0 (assert).

; --- Timing constants for simple busy-wait loops ---
IEC_PRE_SYNC_DELAY  = $28                 	; Pre-sync settle loop count before polling CLOCK IN.
IEC_POST_SYNC_DELAY = $06                 	; Post-sync settle after writing ATN/assert mask.

; --- Working/storage locations ---
iec_shiftreg     	= $4752                	; Shift register for TX/RX bit pairs (ROL into/out of carry).
iec_cmd_track   	= $4753                	; Command parameter: track number to transmit.
iec_cmd_sector  	= $4754                	; Command parameter: sector number to transmit.
cia2_pra 			= $DD00      			; CIA2 Port A (PRA). Read to sample inputs; write masks to drive IEC outputs.
