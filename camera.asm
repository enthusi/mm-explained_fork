 ...
 *       room_scene_left_edge; when exceeded, step one column toward the actor.
 * 
 *   After movement (or if no movement), clamps cam_target_pos to room bounds
 *   and derives the visible span [room_scene_left_edge, room_scene_right_edge].
 *
 ...
 *   room_scene_left_edge             Current left edge (for FOLLOW window tests).
 * Vars:
 ...
 *   room_scene_left_edge             Updated from cam_target_pos (center - VIEW_HALF_SPAN).
 *   room_scene_right_edge            Updated from left_edge (+ VIEW_FULL_SPAN_MINUS1).
 *
 * Flags:
 ...
 *        - Compute (actor_x - room_scene_left_edge) and compare against follow window.
 ...
 *   4) Finally, compute [room_scene_left_edge, room_scene_right_edge] from cam_target_pos.
 *========================================================*/
* = $0668
cam_upd_target:
...
mode_follow_actor:
		...
		// Left-side test:
		//   delta_left := actor_x - room_scene_left_edge
		//   if delta_left < left_follow_threshold → need to pan (actor too far left)
		sec
		sbc     room_scene_left_edge          // A := actor_x - left_edge
